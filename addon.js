...
const manifest = {
  id: "org.stremio.CZSK.multi",
  version: "0.0.1",
  name: "CZSK Multi Source",
  description: "Addon for Stremio that searches multiple Czech sources.",
  resources: ["stream"],
  types: ["movie", "series"],
  idPrefixes: ["tt"],
  catalogs: [],
...
async function getTitleFromWikidata(imdbId) {
  try {
    const baseQuery = (lang) => `SELECT ?filmLabel WHERE { ?film wdt:P345 "${imdbId}". SERVICE wikibase:label { bd:serviceParam wikibase:language "${lang}". } }`;
    ...
    return { czTitle: czResult?.filmLabel?.value, enTitle: enResult?.filmLabel?.value };
  } catch (error) { return {}; }
}
...
addonInterface.streamHandler = streamHandler; // Přidáme handler pro server.js

...
// Schéma pro uživatele s počítadlem zařízení
...
// Schéma pro tokeny s číslem zařízení
...
app.get('/admin', adminAuth, async (req, res) => {
  ...
  users.forEach(u => {
    ...
    if (userTokens.length === 0) {
        ...
        userTokens.forEach(tkn => {
          ...
          const lastWatchedText = tkn.lastWatchedAt ? `${tkn.lastWatchedImdbId} ${tkn.lastWatchedInfo || ''}`.trim() : 'N/A';
          const lastWatchedTime = tkn.lastWatchedAt ? new Date(tkn.lastWatchedAt).toLocaleString() : 'N/A';
  ...
  const html = `
    ...
    <body>
      ...
      <table><tr><th>User</th><th>Expires</th><th>Device MAC</th><th>Last Watched</th><th>Time</th><th>Last IP</th><th>Device Info</th><th>Install Link</th><th>Actions</th></tr>${rowsHtml}</table>
    </body></html>`;
...
app.use(MOUNT_PATH, async (req, res, next) => {
    try {
        ...
        // Vytvoříme unikátní manifest pro tuto konkrétní instalaci
        ...
        // Vytvoříme novou, personalizovanou instanci addon routeru
        const personalizedAddon = new addonBuilder(manifest);
        personalizedAddon.defineStreamHandler(addonInterface.streamHandler);
        const addonRouter = getRouter(personalizedAddon.getInterface());
        ...
        // Předáme požadavek na nově vytvořený, personalizovaný router
        addonRouter(req, res, next);

    } catch (err) {
        console.error("Main middleware error:", err);
